...
from utils.data_loader import Dataset_Union_ALL, Dataset_Union_ALL_Val

parser = argparse.ArgumentParser()
parser.add_argument('-tdp', '--test_data_path', type=str, default='/cache/fast_data_nas8/utkarsh/sam_med3d_cache')
parser.add_argument('-vp', '--vis_path', type=str, default='./results/visualization') # ./results/visualization2d for medsam results
parser.add_argument('-cp', '--checkpoint_path', type=str, default='./work_dir/lidc_finetune/sam_model_dice_best.pth') # ./ckpt/medsam2d_point_prompt.pth for medsam
...
def finetune_medsam2d(img3D, gt3D, sam_model_tune, target_size=1024, click_method='random', device='cuda', num_clicks=1, prev_masks=None):
    pred_list = []
    iou_list = []
    dice_list = []
    ...
    for slice_idx in tqdm(range(img3D.size(-1)), desc="transverse slices", leave=False):
        img2D, gt2D = img3D[..., slice_idx], gt3D[..., slice_idx]
        ...
        img2D = (img2D - img2D.min()) / torch.clamp(img2D.max() - img2D.min(), min=1e-8) # normalize input for medsam model
...
def finetune_model_predict2D(img3D, gt3D, sam_model_tune, target_size=256, click_method='random', device='cuda', num_clicks=1, prev_masks=None):
    ...
    for slice_idx in tqdm(range(img3D.size(-1)), desc="transverse slices", leave=False):
        ...
        img2D, gt2D = img2D.to(device), gt2D.to(device)
        img2D = (img2D - img2D.mean()) / img2D.std() # standardize input
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10, prev_masks=None):
    ...
    for num_click in range(num_clicks):
        with torch.no_grad():
            ...
            medsam_seg = (medsam_seg_prob > 0.5).astype(np.uint8) # confidence threshold 0.5
            pred_list.append(medsam_seg)
...
if __name__ == "__main__":    
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        # if not(img_name == "1.3.6.1.4.1.14519.5.2.1.6279.6001.105495028985881418176186711228"): continue
        sz = image3D.size()
        ...
        if True: # else
            ...
            if(args.dim==3):
                ...
                seg_mask_list, points, labels, iou_list, dice_list = finetune_medsam2d(

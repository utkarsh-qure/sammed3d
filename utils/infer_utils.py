...
def postprocess_masks(low_res_masks, image_size, original_size, ft2d=False):
    # NOTE: set `ft2d`=True for medsam2d
    ...
    if ft2d and ori_h < image_size and ori_w < image_size:
...
def finetune_medsam2d(img3D, gt3D, sam_model_tune, target_size=1024, click_method='random', device='cuda', num_clicks=1, prev_masks=None):
    ...
    click_points = []
    click_labels = []
    for slice_idx in tqdm(range(img3D.size(-1)), desc="transverse slices", leave=False, colour="green"):

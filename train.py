...
parser.add_argument('--task_name', type=str, default='lidc_finetune') # last: lidc_finetune_resampled_turbo
...
def get_dataloaders_32(args):
    train_dataset = Dataset_Union_ALL(paths=img_datas, transform=tio.Compose([
        tio.ToCanonical(),
        tio.Resample((1, 1, 1)),
        tio.CropOrPad(mask_name='label', target_shape=(args.img_size//4, args.img_size, args.img_size)), # crop only object region
        tio.RandomFlip(axes=(0, 1, 2)),
    ]),
    threshold=100)
...
class BaseTrainer:
    def __init__(self, model, dataloaders, args, train_from_scratch:bool=False):
        ...
        if not train_from_scratch:
            init_ckpt_path = os.path.join(self.args.work_dir, self.args.task_name, 'sam_model_latest.pth')
            init_ckpt_path = init_ckpt_path if os.path.exists(init_ckpt_path) else "/home/users/utkarsh.singh/qct/medsam/SAM-Med3D/ckpt/sam_med3d_turbo.pth"# original: "/home/users/utkarsh.singh/qct/medsam/SAM-Med3D/ckpt/sam_med3d.pth"
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.args.num_epochs):
            ...
            epoch_loss, epoch_iou, epoch_dice = self.train_epoch_simple(epoch, num_clicks=11)
...
def main():
    ...
    trainer = BaseTrainer(model, dataloaders, args, train_from_scratch=False) # train_from_scratch is True for any img not of the shape (128, 128, 128)
    trainer.train()

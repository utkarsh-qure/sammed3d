...
class PromptEncoder3D_mod(nn.Module):
    def __init__(
        self,
        embed_dim: int,
        image_embedding_size: Tuple[int, int, int],
        ...
        """
        super().__init__()
        print("running modded prompt encoder...")
        self.embed_dim = embed_dim
        self.input_image_size = input_image_size
        self.image_embedding_size = image_embedding_size
        self.pe_layer = PositionEmbeddingRandom3D(embed_dim // 3)
        ...
        self.mask_input_size = (image_embedding_size[0], image_embedding_size[1], image_embedding_size[2])
        self.mask_downscaling = nn.Sequential(
            nn.Conv3d(1, mask_in_chans // 4, kernel_size=(1, 2, 2), stride=(1, 2, 2)),
            LayerNorm3d(mask_in_chans // 4),
            activation(),
            nn.Conv3d(mask_in_chans // 4, mask_in_chans, kernel_size=(1, 2, 2), stride=(1, 2, 2)),
            LayerNorm3d(mask_in_chans),
            activation(),
            nn.Conv3d(mask_in_chans, embed_dim, kernel_size=1),
        ) # changed, ensures that the dense_embedding shape is (8, 8, 8) instead of (2, 8, 8) when working with (32, 128, 128) images v/s (128, 128, 128)
        self.no_mask_embed = nn.Embedding(1, embed_dim)

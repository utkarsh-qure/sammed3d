...
class ImageEncoderViT3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            try:
                ...
                x = x + self.interpolate_pos_encoding(*x.shape[1:4]) # LBK
        ...
        # for 2D: x = self.neck(x.permute(0, 3, 1, 2))
    ...
    # LBK
    def interpolate_pos_encoding(self, d, h, w):
        depth, height, width = self.pos_embed.shape[1:4]

        patch_pos_embed = nn.functional.interpolate(
            self.pos_embed.permute(0, 4, 1, 2, 3),
            scale_factor=(d/depth, h/height, w/width),
            mode='bicubic',
        ).permute(0, 2, 3, 4, 1)
        return patch_pos_embed


# Only difference wrt Block is that 3D submodules are called instead of their 2D counterparts
...
# just add depth dimension to relevant computations
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.proj(x)
        # B C X Y Z -> B X Y Z C
        x = x.permute(0, 2, 3, 4, 1)  # change to channels last
        return x
